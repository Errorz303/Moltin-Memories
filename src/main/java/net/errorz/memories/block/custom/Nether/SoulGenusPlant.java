...
public class SoulGenusPlant extends NetherPlants implements Fertilizable {
    public static final MapCodec<SoulGenusPlant> CODEC = createCodec(SoulGenusPlant::new);
    private static final float MIN_MOVEMENT_FOR_DAMAGE = 0.5F;
    public static final int MAX_AGE = 3;
    public static final IntProperty AGE;
    private static final VoxelShape SMALL_SHAPE;
    private static final VoxelShape LARGE_SHAPE;

    public MapCodec<SoulGenusPlant> getCodec() {
        return CODEC;
    }

    public SoulGenusPlant(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState((BlockState)((BlockState)this.stateManager.getDefaultState()).with(AGE, 0));
    }

    public ItemStack getPickStack(WorldView world, BlockPos pos, BlockState state) {
        return new ItemStack(MemoryItems.SOUL_PEPPER);
    }
    protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
        return super.canPlantOnTop(floor, world, pos) || floor.isOf(Blocks.SOUL_SOIL) || floor.isOf(Blocks.SOUL_SAND) || floor.isOf(Blocks.MAGMA_BLOCK);
    }
    ...
    protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        if (entity instanceof LivingEntity && entity.getType() != EntityType.MAGMA_CUBE && entity.getType() != EntityType.WITHER_SKELETON) {
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (i > 1) {
            ...
            dropStack(world, pos, new ItemStack(MemoryItems.SOUL_PEPPER, j + (bl ? 1 : 0)));
